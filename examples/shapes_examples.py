...
from tempe.shapes import Rectangles, Lines, HLines, VLines, Polygons, Circles, Ellipses, BLIT_KEY_RGB565
...
labels = Text(
    [
        (4, 4), (4, 44), (4, 84), (4, 124), (4, 184),
        (164, 4), (164, 44), (164, 124), (164, 184),
    ],
    Repeat(0x0000),
    [
        "Lines", "HLines", "VLines", "Polygons", "Circles",
        "Markers", "Bitmaps", "Rectangles", "Ellipses"
    ],
    font=font,
)
...
# draw some polygons
stars = [[
    (16 + x + int((16 - 8 * (a % 2)) * math.sin(math.pi*a/5)), 160 - int((16 - 8 * (a % 2)) * math.cos(math.pi*a/5)))
    for a in range(10)
]
...
star = array('h', [
    ...
    for a in range(10)]
    for x in p])
smiley = framebuf.FrameBuffer(array('B', [
    0b00111100,
    0b01111110,
    0b11011011,
    0b11011011,
